#!/usr/bin/env python3
"""Compare Python and JavaScript debug trees for Quenya Tengwar transcription.

This module compares the internal transcription tree structures generated by
the Python and JavaScript implementations of Glaemscribe for the phrase
'Ai ! laurië lantar lassi súrinen ,'. It identifies differences in node
properties, missing nodes, and generates a detailed report.

The comparison includes:
    - Flattening both tree structures for easier comparison
    - Identifying nodes that differ between implementations
    - Finding nodes present in only one implementation
    - Reporting differences in character, replacement, and effective status
    - Saving results to a diff file for further analysis
